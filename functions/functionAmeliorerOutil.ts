...
export function functionAmeliorerOutil(){
    const faire = zeroNull("\nQue voulez vous améliorer sur votre \x1b[34moutil\x1b[0m ?\n\x1b[34mtaille de la récolte\x1b[0m : "+joueurs.outil.prixAmelioration+"€\n")
    if(faire =="1" || faire.toLowerCase() === "taille de la recolte"){
        if(joueurs.monaie >= joueurs.outil.prixAmelioration){
            const temp = joueurs.outil.prixAmelioration
            joueurs.monaie -= joueurs.outil.prixAmelioration
            joueurs.outil.augmenterLaTailleDeLaRecolte()
            console.log("\n\x1b[32mVous venez d'augmenter la taille de votre recolte à "+joueurs.outil.tailleDeLaRecolte+" \x1b[0mpour\x1b[31m "+temp+"€\x1b[0m")
        }else{
            console.log(`\n\x1b[31mVous n'avez pas assez d'argent (${joueurs.monaie}/${joueurs.outil.prixAmelioration}€) !\x1b[0m\n`)
        }
    }else if (faire == "0"){
        return
    }else{
